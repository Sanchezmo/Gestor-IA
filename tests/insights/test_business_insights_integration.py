...
class TestCustomerInsightsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_customer_invoice_summary_integration(
        self, 
        context_a, 
        user_context
    ):
...
class TestSupplierInsightsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_supplier_invoice_summary_integration(
        self, 
        context_a, 
        user_context
    ):
...
class TestPartialPayment:
    ...
    async def test_partial_payment(self):
        ...
        from core.hermes.instance_config import InstanceConfig, DatabaseConfig, DolibarrConfig, DomainConfig, TelegramConfig, AIConfig
        from core.hermes.tools import tool_registry
        from core.hermes.identity import UserContext, DolibarrUser, DolibarrGroup
        from core.hermes.instance_config import (
            InstanceConfig,
        ...
        config = InstanceConfig(
            ...
            dolibarr=DolibarrConfig(version="23", internal_url="http://localhost", api_key="key", documents_path="/tmp", timezone="Europe/Madrid"),
            ...
            enabled_tools=["list_customer_invoices", "search_customer_invoices", "get_customer_invoice", "count_customer_invoices"],
        ).resolve_paths()
        ...
        with patch.object(tool_registry, 'execute_tool', new_callable=AsyncMock) as mock:
            ...
            result = await service.customer_outstanding_summary(
                company_context=CompanyContext(instance_config=InstanceConfig(
                    instance_id="test",
                    company_name="Test",
                    database=DatabaseConfig(host="127.0.0.1", port=3306, name="test", user="test", password="test"),
                    dolibarr=DolibarrConfig(version="23", internal_url="http://localhost", api_key="key", documents_path="/tmp", timezone="Europe/Madrid"),
