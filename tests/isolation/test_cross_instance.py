...
from core.hermes.instance_config import InstanceConfig, DolibarrConfig, TelegramConfig, DomainConfig, AIConfig, AIPolicyScope
from core.hermes.context import CompanyContext, create_test_context
from core.hermes.resolver import resolve_instance_config, lookup_instance_by_domain, lookup_instance_by_api_key, _build_domain_index, invalidate_domain_cache
from core.hermes.extensions import ExtensionRegistry, extension_registry
from core.integrations.dolibarr.client import DolibarrClient
from core.integrations.telegram.client import TelegramClient, MockTelegramClient
...
def context_a(instance_a_config):
    return create_test_context("empresa_a", "Empresa A S.L.", "telegram_user", "user_1")


@pytest.fixture
def context_b(instance_b_config):
    return create_test_context("empresa_b", "Empresa B S.L.", "telegram_user", "user_2")
...
class TestInstanceResolverIsolation:
    ...
    async def test_resolve_by_api_key(self):
        """API key gsk_empresa_a_... resuelve a empresa_a."""
        request = MagicMock()
        request.headers = {"Authorization": "Bearer gsk_empresa_a_abc123"}
        request.url.path = "/api/test"
...
class TestExtensionRegistryIsolation:
    ...
    def test_agents_registered_per_instance(self):
        ...
        def make_agent_a(config): return "agent_a"
        def make_agent_b(config): return "agent_b"
    ...
    def test_tools_registered_per_instance(self):
        ...
        async def tool_a(): return "result_a"
        async def tool_b(): return "result_b"
...
class TestCloudflareIngressIsolation:
    ...
    async def test_ingress_validation_rejects_cross_instance(self, instance_a_config, instance_b_config):
        ...
        assert any("not configured" in e for e in validation["errors"])
...
class TestInstanceConfigValidation:
    ...
    def test_instance_id_validation_rejects_invalid(self):
        with pytest.raises(ValueError):
            InstanceConfig(
                ...
                company_name="Test",
                database=DolibarrConfig(internal_url="http://x", api_key="k", documents_path="/d", db_name="d", db_user="u", db_password="p"),
    ...
    def test_instance_id_validation_rejects_reserved(self):
        for reserved in ["global", "shared", "core", "instances", "companies", "scripts", "tests", "config", "infrastructure"]:
            with pytest.raises(ValueError):
                InstanceConfig(
