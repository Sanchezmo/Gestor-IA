    ...
    async def test_happy_path_list_thirdparties(self, context_a, telegram_identity_a, dolibarr_user_a):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("core.hermes.identity_store.get_instances_root", return_value=Path(tmpdir)):
                ...
                with patch("core.hermes.context.CompanyContext.create_dolibarr_client_for_user", return_value=mock_client):
...
class TestWebhookSecretValidation:
    ...
    async def test_invalid_secret_rejected_before_identity_resolver(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # For unit test: verify tool has NO required_permissions (Dolibarr is sole ERP authority)
            tool = ListThirdpartiesTool()
...
# =========================================================================
# TESTS E2E - SIN PERMISO ERP (Dolibarr 403)
# =========================================================================


class TestNoERPPermission:
    """Tests para autorización denegada por Dolibarr (403)."""

    @pytest.mark.asyncio
    async def test_user_without_thirdparty_read_gets_403(self, context_a, telegram_identity_a, dolibarr_user_no_perms):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("core.hermes.identity_store.get_instances_root", return_value=Path(tmpdir)):
                ...
                # Mock Dolibarr returns 403 for list_thirdparties
                mock_client = make_mock_dolibarr_client(
                ...
                # Tool executes, Dolibarr returns 403, tool maps to safe error
                with patch("core.hermes.context.CompanyContext.create_dolibarr_client_for_user", return_value=mock_client):
                    tool = ListThirdpartiesTool()
                    result = await tool.execute(context_a, user_context, limit=10, page=1)

                assert result.success is False
                assert result.error_code == "DOLIBARR_PERMISSION_DENIED"
                assert "permisos en Dolibarr" in result.error_message

                # Dolibarr was called (no Hermes-level permission check)
                mock_client.list_thirdparties.assert_called_once()
    ...
    async def test_dolibarr_401_returns_safe_message(self, context_a, telegram_identity_a, dolibarr_user_a):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("core.hermes.identity_store.get_instances_root", return_value=Path(tmpdir)):
                ...
                assert result.success is False
                assert result.error_code == "DOLIBARR_AUTH_FAILED"
                assert "No he podido autenticar tu usuario en Dolibarr" in result.error_message
                # NO API key leaked
...
class TestAuditLogging:
    ...
    @pytest.mark.asyncio
    async def test_erp_permission_denied_audits_dolibarr_error(
        self, context_a, telegram_identity_a, dolibarr_user_no_perms
    ):
        """Dolibarr 403 genera ToolResult con error_code DOLIBARR_PERMISSION_DENIED."""
        import tempfile
        from pathlib import Path
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("core.hermes.identity_store.get_instances_root", return_value=Path(tmpdir)):
                ...
                assert result.metadata["instance_id"] == "empresa_a"
                assert result.metadata["dolibarr_user_id"] == 99
...
class TestToolRegistry:
    ...
    async def test_tool_registry_get_list_thirdparties(self):
        ...
        # Tools have NO required_permissions for ERP - Dolibarr is sole authority
        assert tool.required_permissions == frozenset()

    @pytest.mark.asyncio
    async def test_tool_registry_execute_tool_not_found(self, context_a, telegram_identity_a, dolibarr_user_a):
    ...
    @pytest.mark.asyncio
    async def test_tool_registry_execute_dolibarr_403(self, context_a, telegram_identity_a, dolibarr_user_no_perms):
        """Dolibarr 403 → tool_registry returns DOLIBARR_PERMISSION_DENIED."""
        import tempfile
        from pathlib import Path
