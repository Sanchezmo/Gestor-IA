...
class DolibarrClient:
    ...
    async def find_thirdparty_by_tax_id(
        ...
        while pages_checked < max_pages:
            ...
            for party in parties:
                party_vat = self._normalize_tax_id(
                    party.get("vat_number", "") or party.get("vatnumber", "")
                )
    ...
    async def add_supplier_invoice_line(
        self, invoice_id: int, line_data: dict[str, Any]
    ) -> dict[str, Any]:
...
async def get_dolibarr_client(
    ...
    if ctx is None:
        ...
        raise RuntimeError("get_dolibarr_client requiere CompanyContext. Usa Depends(get_company_context) en el endpoint.")
