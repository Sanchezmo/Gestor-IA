...
class CloudflareManager:
        ...
        for instance in instances:
            ...
            if domains.hermes:
                ingress_rules.append({
                    ...
                    "service": f"http://127.0.0.1:8000",  # Hermes Core puerto único
                    "originRequest": {
                        "connectTimeout": "30s",
                        "noTLSVerify": True,
                    },
    ...
    async def ensure_dns_for_instance(self, instance: "InstanceConfig", server_ip: str) -> dict:
        ...
        for rtype, name, content, proxied in records_to_ensure:
            try:
                # Verificar si existe
                existing = await self.adapter.list_dns_records(
                    self.zone_id, type=rtype, name=name
                )
...
def create_cloudflare_manager(global_settings=None) -> Optional[CloudflareManager]:
    """Crear CloudflareManager desde GlobalSettings."""
    if global_settings is None:
        from core.hermes.config import get_global_settings
        global_settings = get_global_settings()
    
    if not all([global_settings.CLOUDFLARE_API_TOKEN, global_settings.CLOUDFLARE_ACCOUNT_ID, global_settings.CLOUDFLARE_ZONE_ID]):
        return None
